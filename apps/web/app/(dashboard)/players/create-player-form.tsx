...
import { createPlayer, type ActionState } from "./actions";
...
export function CreatePlayerForm({ stores }: { stores: { id: string; name: string }[] }) {
  const [state, formAction] = useFormState<ActionState, FormData>(createPlayer, {
    error: null,
    success: null,
  });
  ...
  return (
    <form action={formAction} className="flex flex-wrap items-end gap-3 rounded-lg border border-ink-700 bg-ink-900 p-4">
      <div className="min-w-[180px] flex-1">
        ...
        <input id="name" name="name" required className={inputClass} placeholder="Xiaomi Player 01" />
      </div>
      ...
      <div>
        ...
        <select id="type" name="type" defaultValue="ANDROID" className={inputClass}>
